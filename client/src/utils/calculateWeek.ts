 ...
 * @return an array of 7 dates. The dates will be used to fetch the workouts on those days which will
 * then be displayed in the Week view 
 */
export function calculateWeek(dateArr: string[]): string[] {
  const numDaysInMonth = new Date(Number(dateArr[2]), Number(dateArr[0]), 0).getDate();
  let numDaysPrevMonth = null;
  if (Number(dateArr[0]) === 1) {
  ...
  const weekObj = {};
  const dateArrAsString = `${dateArr[2]}-${String(dateArr[0]).padStart(2, '0')}-${String(dateArr[1]).padStart(2, '0')}T00:00:00`;
